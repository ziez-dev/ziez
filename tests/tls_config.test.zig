...
test "TlsConfig default values" {
    const config = ziez.TlsConfig{ .cert = .{ .pem_bytes = "dummy" }, .key = .{ .pem_bytes = "dummy" } };
...
test "TlsConfig client_auth variants" {
    const c1 = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .client_auth = .none };
    try std.testing.expectEqual(ziez.ClientAuth.none, c1.client_auth);
    const c2 = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .client_auth = .request };
    try std.testing.expectEqual(ziez.ClientAuth.request, c2.client_auth);
    const c3 = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .client_auth = .require };
    try std.testing.expectEqual(ziez.ClientAuth.require, c3.client_auth);
}

test "TlsConfig with mTLS CA" {
    const config = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .client_auth = .require, .client_ca = .{ .pem_bytes = "ca-pem" } };
...
test "TlsConfig with SNI hostnames" {
    ...
    const config = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .sni_hostnames = &hostnames };
    try std.testing.expect(config.sni_hostnames != null);
    try std.testing.expectEqual(@as(usize, 2), config.sni_hostnames.?.len);
}

test "TlsConfig cipher suite ordering" {
    const config = ziez.TlsConfig{ .cert = .{ .der_bytes = "x" }, .key = .{ .der_bytes = "x" }, .cipher_suites = &.{ .CHACHA20_POLY1305_SHA256, .AES_128_GCM_SHA256, .AES_256_GCM_SHA384 } };
...
test "App.tls() stores config" {
    ...
    defer app.deinit();
    app.tls(.{ .cert = .{ .pem_bytes = "cert" }, .key = .{ .pem_bytes = "key" } });
    try std.testing.expect(app.tls_config_ptr != null);
    try std.testing.expect(app.handle_tls_fn != null);
}
...
test "RedirectHttpConfig defaults and exclusions" {
    ...
    const excluded = ziez.RedirectHttpConfig{ .exclude = &.{ "/health", "/.well-known/acme-challenge" } };
...
test "App.redirectHttp() stores config" {
    var app = ziez.App.init(std.testing.allocator);
    defer app.deinit();
    app.redirectHttp(.{ .port = 8080, .to = 3443, .exclude = &.{"/health"} });
    try std.testing.expect(app.redirect_config_ptr != null);
    try std.testing.expect(app.run_redirect_fn != null);
}

test "App.reloadTls() updates stored config before runtime exists" {
    var app = ziez.App.init(std.testing.allocator);
    defer app.deinit();
    try app.reloadTls(.{ .cert = .{ .pem_bytes = "next-cert" }, .key = .{ .pem_bytes = "next-key" }, .min_version = .tls_1_3 });
    try std.testing.expect(app.tls_config_ptr != null);
}
